function [results, Eventi] = analisi_Nframes011_multiEvento_007_NOpV_adp(frame_start, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%(coord, value)
Nframes = length(mtotalDT(1,1,:)) - frame_start; %T = [1,2,3,4,5,6,7,8,9], DT = [4-1,5-2,6-3,7-4,8-5,9-6], frame_start = 3 => DT = [7-4,...]
...
for i = 0 : Nframes - 1 %cafone
    ...
    fname = ['frame', num2str(frame_start+ i + fr_diff)]; %T = [1,2,3,4,5,6,7,8,9], DT = [4-1,5-2,6-3,7-4,8-5,9-6]
    
    %calcolo tempo                                         % DT = [4-1,5-2,6-3,7-4,8-5,9-6]
    framestates(i+1, 6) = times(1, frame_start + i);       % timesDT = [3/30,4/30,5/30,6/30,7/30,8/30]
                                         % DD ha aggiunto +1 %                     |
    %decomposizione            % DD ha aggiunto +1
    mdiff = mtotalDT(:,:, frame_start + i); % DT = [4-1,5-2,6-3,7-4,8-5,9-6]
