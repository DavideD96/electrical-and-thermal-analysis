function [max_evento, min_evento, area_max, area_min, imsov] = detection_multiEvento_003(max_hotspot, min_hotspot, point_state_max, point_state_min, imrec, Rows, Columns)
    area_max = zeros(0,1);
    area_min = zeros(0,1);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        while j <=  (length(max_evento(:,1)))
            ...
            while i <= length(max_evento(:,1))
                ...
                if BWfinal_max(y_controllo, x_controllo) == 1
                    %primi vicini
                    state = primi_vicini(max_evento(i,:), 1, imrec);
        ...
        while j <= (length(min_evento(:,1)))
            ...
            while i <= length(min_evento(:,1))
                ...
                if BWfinal_min(y_controllo, x_controllo) == 1
                    %primi vicini
                    state = primi_vicini(min_evento(i,:), 0, imrec);
