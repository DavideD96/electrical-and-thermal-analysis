...
[~,ia,ix] = unique(eventiBoth_cold_hot(:,2)); %deltaT>0
C = accumarray(ix,1).';

eventiBoth_cold_hot(ia,k) %coordinata da plottare con spessore dato da  


for j = 1:size(matriceEventi,1)
    for k = 1:size(matriceEventi,1)
        if matriceEventi(k,2) ~= 0
            y = ceil(matriceEventi(k,2)/rows);  
            x = matriceEventi(k,2) - rows*(y-1);
            plot(y,x,'k+', 'MarkerSize', 20);
        end
        if matriceEventi(k,3) ~= 0
            y = ceil(matriceEventi(k,2)/rows);  
            x = matriceEventi(k,2) - rows*(y-1);
            plot(y,x,'k+', 'MarkerSize', 20);
        end
