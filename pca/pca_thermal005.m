function pc2 = pca_thermal005(folder, filename,fileres, varargin)
...
for k = 1:2:num
    ...
    elseif prod(varargin{k}=='smoothing_before')
        smooth = varargin{k+1};
    elseif prod(varargin{k}=='start_end_frames')
        start_end = varargin{k+1};
    end
end

check = exist('start_end_pc.mat','file');
if check ~= 0
    start_end = load('start_end_pc.mat','-mat');
    start_end = cell2mat(struct2cell(start_end));
else
    save('start_end_pc.mat','start_end','-mat')
end
...
sgtitle(append('allPC__f',folder))

savefig(PC, append(filename,'_PC_','.fig'));

PC1 = figure;
hold on;
title(append('PC1__f',folder));
%first principal component
...
savefig(PC1, append(filename,'_PC1_','.fig'));

path = pwd;
cd ..
cd ..
cd lettureCoppie
saveas(PC1, append(folder,'_PC1_','.png'),'png');
saveas(PC, append(folder,'_PC_','.png'),'png');
cd(path)
