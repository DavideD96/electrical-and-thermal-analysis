function hotspot_1 (filename, fr_evento, fr_diff, soglia_max, soglia_min, coordname, varargin)
num = length(varargin);
...
figure (2)
    ...
    if smooth == 1
       hold on
            ...
            %Trova massimo
            max_hotspot = find(imregionalmax(z));
            max_hotspot = max_hotspot(z(max_hotspot) >= soglia_max);
            plot3(data_x(max_hotspot),data_y(max_hotspot), z(max_hotspot),'r*','MarkerSize',24);

            %Trova minimo
            min_hotspot = find(imregionalmin(z));
            min_hotspot = min_hotspot(z(min_hotspot) <= soglia_min);
            plot3(data_x(min_hotspot),data_y(min_hotspot), z(min_hotspot),'b*','MarkerSize',24);
