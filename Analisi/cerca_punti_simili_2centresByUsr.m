function [group1, group2] = cerca_punti_simili_2centresByUsr(Eventi, frame_start, fr_diff, coord1, coord2)
...
group1(:,1) = times(1:end-1);

for i = bias : length(fieldnames(Eventi_supp))+bias-1   %ciclo su tutti i frames
    fname = ['frame', num2str(i)];
    group1(i-bias+1,2) = match_evento_area_001(Eventi_supp.(fname),coord1,1);
end

%secondo gruppo
group2 = zeros(length(fieldnames(Eventi_supp)),2);
group2(:,1) = times(1:end-1);

for i = frame_start+fr_diff : length(fieldnames(Eventi_supp))+bias-1   %ciclo su tutti i frames
    fname = ['frame', num2str(i)];
    group2(i-bias+1,2) = match_evento_area_001(Eventi_supp.(fname),coord2,1);
end
