...
peak_max = zeros(length(frames_states(:,1)), 3);
peak_min = zeros(length(frames_states(:,1)), 3);
...
while j <= length(frames_states(:,8))
    ...
    if (n_evento ~= 0)
        ...
        peak_max(max_peaks_array(1,4),:) = max_peaks_array(1, 1:3);
        peak_min(min_peaks_array(1,4),:) = min_peaks_array(1, 1:3);
