function ica_thermal001(filename,fileres,start_end, q, varargin)
...
Model = rica(data_,q);
Model_Sp = sparsefilt(data_,q);

figure
for i = 1:q/3
    for j = 1:3
        subplot(q/3,3,3*(i-1)+j);
        pc1 = reshape(Model.TransformWeights(:,3*(i-1)+j),[rows,col]);
...
for i = 1:q/3
    for j = 1:3
        subplot(q/3,3,3*(i-1)+j);
        pc1 = reshape(Model_Sp.TransformWeights(:,3*(i-1)+j),[rows,col]);
        title_str = num2str(3*(i-1)+j);
        title(title_str);
        imagesc(pc1);
        xlim([1,col]);
