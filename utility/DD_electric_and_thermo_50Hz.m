function DD_electric_and_thermo_50Hz(filename, varargin)
...
if video == 1
    ...
    vidObj.FrameRate = RframeRate/rallenty;
    open(vidObj);
...
termo_fr_over_res_fr = 2.5; %0.5;

for i=1:nFrames
    termo_times(i)=(i-1)*1/(termo_fr_over_res_fr*RframeRate+0.2);
end

if termo_fr_over_res_fr > 1
    video_length = nFrames*2; %considero solo caso a 50 Hz, per cui se campiono resistenza a 20 Hz => 100 Hz
    video_frameRate = 100;
else
    video_length = nres;
    video_frameRate = 20;
...
for i=1:delay*2 %fattore moltiplicativo per matchare la frequenza
    ...
    title('Electrical conductance [\Omega^{-1}]','Parent',axres,'Position',[500 500])
    title('Temperature [°C]','Parent',axtemp)
    title('Difference between frames [°C]','Parent',axdiff)

    for j=1:2 %%% cafone, serve a fare 2 frame uguali in successione
