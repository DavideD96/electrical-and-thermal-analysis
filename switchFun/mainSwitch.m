...
for j = 1:i
    ...
    elseif opt(j)== "cdt"
        ...
        justSwitches = anlR(anlR(:,end-1)~=0, :);
        %size(justSwitches)
        ...
        s.cnt = v;        
        %----shuffled switches----
        random01 = anlR(randperm(size(anlR,1)),end-1); 
        ...
        xlim([anlR(1,1),anlR(end,1)]);
        hold on;
        notShuffled = anlR(anlR01(:,end)==1,:);
        gridxy(notShuffled(:,1), 'Color','k');
        %xline(notShuffled(:,1), 'Color','k');
